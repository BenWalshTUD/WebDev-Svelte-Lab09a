	...
	register: async ({ request, locals, cookies }) => {
		try {
			...
			const userData = {
				...
				dob: formData.get('dob'),          // YYYY-MM-DD
				email: formData.get('email'),
				role: 'user'
			};

			const { user, session, sessionToken } = await authService.register(userData);
			...
			return fail(500, {
				errors: { general: err instanceof Error ? err.message : 'Registration failed' }
			});
		}
	}
};